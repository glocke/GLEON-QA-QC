...
% there.
initValID = 154690024; %IMPORTANT
currentValID = 0; % stores the last valueID cleaned

if initValID == 154690024 % remember need to change this, for first startup IMPORTANT
...
timeToWait = 60; % time before clean occurs again, in seconds
moveOn = 1;
...
%        catch exception
 ...
 %   end
    ...
    if size(D.QResult,1) > 0
        ...
        %while tryagain
            %try
                [ newYearFrac newData newStreamDataArray] = RangeChecks(D.YearFrac, Removable, Streams, TS, streamDataArray);
                %tryagain = 0;
            %catch exception
               % pause(60)
               % tryagain = 1;
    ...
    clearvars -except records ticmajor TimeFormat Interactive ...
        ...
        Interpolate Filter TargetPeriod timeToWait moveOn streamDataArray
    pause(timeToWait); % Wait a minute, clean again.
end